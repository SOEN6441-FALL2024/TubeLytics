...
public class ChannelProfileActor extends AbstractActor {
  ...
  public Receive createReceive() {
    return receiveBuilder().match(FetchChannelProfile.class, this::onFetchChannelProfile).build();
  }
  ...
  private void onFetchChannelProfile(FetchChannelProfile message) {
    ...
    CompletionStage<ChannelInfo> channelInfoFuture =
        youTubeService
            .getChannelInfoAsync(message.channelId)
            .exceptionally(
                ex -> {
                  logger.error("Failed to fetch channel info for ID: {}", message.channelId, ex);
                  return new ChannelInfo(
                      "Unavailable", "No description available", 0, 0, 0, message.channelId);
                });

    CompletionStage<List<Video>> videosFuture =
        youTubeService
            .getLast10VideosAsync(message.channelId)
            .exceptionally(
                ex -> {
    ...
    channelInfoFuture
        .thenCombine(videosFuture, ChannelProfileData::new)
        .thenAccept(
            data -> {
        ...
        .exceptionally(
            ex -> {
              logger.error(
                  "Failed to fetch combined data for channel ID: {}", message.channelId, ex);
  ...
  public static class ChannelProfileData {
    ...
    public String toString() {
      return "ChannelProfileData{" + "channelInfo=" + channelInfo + ", videos=" + videos + '}';
    }
  }
}
