...
public class YouTubeServiceActor extends AbstractActor {
    ...
    private void fetchAndStreamResults(String query, ActorRef sender) {
        ...
        videos.whenComplete((results, error) -> {
            if (error != null) {
                System.err.println("Error fetching videos for query '" + query + "': " + error.getMessage());
                sender.tell(new Messages.SearchResultsMessage(query, new ArrayList<>()), getSelf());
            } else {
                // Filter out already processed videos
                List<Video> newResults = results.stream()
                ...
                if (!newResults.isEmpty()) {
                    System.out.println("YouTubeServiceActor sending " + newResults.size() + " results for query: " + query);
